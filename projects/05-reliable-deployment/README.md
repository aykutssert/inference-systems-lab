...
GitHub Actions creates a temporary kind cluster and validates every committed
manifest against its Kubernetes API server. CI also rejects committed
Kubernetes `Secret` manifests. Local validation uses the Docker Desktop API
server:
